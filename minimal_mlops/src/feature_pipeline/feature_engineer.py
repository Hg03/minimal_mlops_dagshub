...
class feature_engine:
    def __init__(self, config: Dict):
        ...
        self.preprocessed_data = self.preprocess_raw(raw_data=self.get_raw())
        
    def get_raw(self) -> pl.DataFrame:
        return load_data_from_supabase(config=self.config, raw_data_path=self.raw_data_path)
    
    def preprocess_raw(self, raw_data: pl.DataFrame) -> pl.DataFrame:
        split_data(config=self.config, raw_data=raw_data)
